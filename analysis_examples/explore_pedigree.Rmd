...
run_wfl <- function (...) {
    ...
    # system2("../src/wflandscape_pedigree", unlist(params), stdout=TRUE)
    read.table( pipe( paste( c("../src/wflandscape_pedigree", unlist(params)), collapse=' ') ) , sep=' ', header=TRUE )
}
...
#' return ped-structured df but with each row giving information about the first parent of the correp row in ped
get_p1 <- function (ped) {
    p1 <- ped[,c("gen","dip")]
    p1$gen <- p1$gen-1L
    p1$.ord <- 1:nrow(p1)
    p1 <- merge( p1, ped, sort=FALSE, all.x=TRUE )
    return( p1[p1$.ord,setdiff(colnames(p1),".ord")] )
}
...
get_p2 <- function (ped) {
    ...
    p2$gen <- p2$gen-1L
    p2$.ord <- 1:nrow(p2)
    p2 <- merge( p2, ped, sort=FALSE, all.x=TRUE )
    return( p2[p2$.ord,setdiff(colnames(p2),".ord")] )
}
