...
ms_to_matrix <- function (msout) {
    ...
    nsegsites <- as.numeric(gsub("segsites: ", "", msout[segsites.ind]))
    ...
    if (nsegsites[1]>0) { 
        stopifnot( 2*nrow(xy) == diff(segsites.ind)-3 )
        neutrals <- list( pos=positions[[1]], geno=get_geno( msout[seq(segsites.ind[1]+2,segsites.ind[2]-2)] ) )
    } else {
        neutrals <- NULL
    }
    if (nsegsites[2]>0) { 
        stopifnot( 2*nrow(xy) == length(msout)-positions.ind[length(positions.ind)] )
        selecteds <- list( pos=positions[[length(positions)]], geno=get_geno( msout[seq(positions.ind[length(positions.ind)]+1,length(msout))] ) )
    } else {
        selecteds <- NULL
    }
    return( list(
...
run_wfl <- function (...) {
    ...
    params <- list(
            ...
            "radius" = 0.04,                  # Radius in which to search for mates.
            "dispersal" = 0.04,               # std. deviation in offspring dispersal
            "seed" =  1234,                  # RNG seed.
            "format" = 1000                  # number of samples to output the genotypes of
        )
...
Here's where the individuals ended up:
```{r plot_locs}
plot(msmat$xy, xlim=c(0,1), ylim=c(0,1))
```
